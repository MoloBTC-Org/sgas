...
Foundations cover why sovereignty matters. The agent guide covers how to build. This document hardens the **tool layer** (MCPs) so a local-first setup still has real security controls.
...
Use an authenticated, scoped, logged MCP server for any tool an agent can call. Prefer local stdio. Treat this layer as production infrastructure, not an experiment.
